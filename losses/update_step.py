...
@partial(jit, static_argnames=["sde", "loss_function", "grad_clip", "nn_model", "optimizer", "get_y_obs"])
def update_step(
    rng, loss_function, sde, nn_model, nn_params, ts, initial_samples, opt_state, optimizer, y_obs, get_y_obs, grad_clip=1.0
):
    val, grads = jax.value_and_grad(loss_function, argnums=(3))(
        rng, sde, nn_model, nn_params, ts, initial_samples, y_obs, get_y_obs
    )
